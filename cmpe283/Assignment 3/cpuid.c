...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if (eax == 0x4fffffff){
		...
		endExit = rdtsc();
		totalExitTime = endExit - startExit;
		// hi-lo seperation logic taken from cmpe283-1.c pr_info function
		ebx = (uint32_t) (totalExitTime >> 32);
		ecx = (uint32_t) (totalExitTime);
	}
	else if (eax == 0x4ffffffd){
		// case 3 from assignment
		eax = exitForEach[exitHandlerInd];
		ecx = exitHandlerInd;
		
		// in case it is one of the exits that isn't supposed to trigger but somehow gets triggered
